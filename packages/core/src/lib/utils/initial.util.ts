...
import differentLocales from '../consts/locale.consts';
import { ModelCommon } from '../models';
import { AppConfigState } from '../states';
import { CORE_OPTIONS } from '../tokens/options.token';

export function configureOAuth(injector: Injector, options: ModelCommon.RootOptions) {
  const fn = () => {
...
export function getInitialData(injector: Injector) {
  const fn = () => {
    ...
    const { skipGetAppConfiguration } = injector.get(CORE_OPTIONS) as ModelCommon.RootOptions;
...
function checkAccessToken(store: Store, injector: Injector) {
  const oAuth = injector.get(OAuthService);
  if (oAuth.hasValidAccessToken() && !store.selectSnapshot(AppConfigState.getDeep('currentUser.id'))) {
    oAuth.logOut();
  }
}
...
export function localeInitializer(injector: Injector) {
  const fn = () => {
    ...
    const lang = store.selectSnapshot(state => state.SessionState.language) || 'zh-Hans';
